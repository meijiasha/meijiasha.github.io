...
- **側邊欄文案 (Sidebar Copy)**:
    - **隨機推薦**: 按鈕文字更新為「推薦店家隨機抽卡」。
    - **推薦附近**: 按鈕文字更新為「附近呷啥隨機抽卡」。
    - **推薦結果**: 標題文字更新為「推薦店家隨機抽卡結果」。
...
- **顏色標準化**: 將全站主色調 (Primary Orange) 統一為 `#ef962e`。更新了 `tailwind.config.js` 與 `index.css`。
    ...
    - **地址格式優化**: 強制 Google Maps API 使用 `zh-TW` 語系。自動填入時，地址欄位直接使用 Google 回傳的完整格式 (`formatted_address`) 以確保包含門牌號碼，同時解析 `address_components` 來自動填入縣市與行政區欄位。
